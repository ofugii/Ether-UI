...
local function makeSection(parent, title)
    ...
    local function _addButton(cfg, target, isSub)
        target = target or body
        local h = isSub and 22 or 36
        local fs = isSub and 11 or 14
        local wr = new("Frame", {Size = UDim2.new(1, 0, 0, h), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
        local btn = new("TextButton", {Position = UDim2.new(0, 10, 0.5, -(h/2)), Size = UDim2.new(1, -20, 0, h-4), BackgroundColor3 = T.B1, Text = cfg.Name or "", TextColor3 = T.MT, TextSize = fs}, wr); cr(4, btn)
    ...
    function api:AddButton(cfg, target) _addButton(cfg, target, false) end
    function api:AddSubButton(cfg, target) _addButton(cfg, target, true) end

    local function _addToggle(cfg, target, isSub)
        target = target or body
        local flg = cfg.Flag or cfg.Name
        local on = (Flags[flg] ~= nil) and Flags[flg] or (cfg.Default == true); Flags[flg] = on
        local h = isSub and 24 or 34
        local fs = isSub and 12 or 14
        local row = new("Frame", {Size = UDim2.new(1, 0, 0, h), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
        local hbg = new("Frame", {Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = T.B4, BackgroundTransparency = 1}, row); cr(4, hbg)
        tl({Position = UDim2.new(0, 12, 0, 0), Size = UDim2.new(1, -66, 1, 0), BackgroundTransparency = 1, Text = cfg.Name, TextColor3 = T.TX, TextSize = fs, TextXAlignment = Enum.TextXAlignment.Left}, row)
    ...
    function api:AddToggle(cfg, target) return _addToggle(cfg, target, false) end
    function api:AddSubToggle(cfg, target) return _addToggle(cfg, target, true) end

    function api:AddCheckbox(cfg, target)
        target = target or body
        local flg = cfg.Flag or cfg.Name
        local on = (Flags[flg] ~= nil) and Flags[flg] or (cfg.Default == true); Flags[flg] = on
        local row = new("Frame", {Size = UDim2.new(1, 0, 0, 34), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
        local hbg = new("Frame", {Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = T.B4, BackgroundTransparency = 1}, row); cr(4, hbg)
    ...
    function api:AddSlider(cfg, target)
        target = target or body
        local flg = cfg.Flag or cfg.Name
        ...
        local wr = new("Frame", {Size = UDim2.new(1, 0, 0, 50), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
    ...
    function api:AddDropdown(cfg, target)
        target = target or body
        local flg = cfg.Flag or cfg.Name
        local dv = Flags[flg] or cfg.Default or (cfg.Items and cfg.Items[1]) or ""; Flags[flg] = dv
        local wr = new("Frame", {Size = UDim2.new(1, 0, 0, 50), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil, ClipsDescendants = false, ZIndex = 5}, target)
    ...
    function api:AddTextBox(cfg, target)
        target = target or body
    ...
    function api:AddColorPicker(cfg, target)
        target = target or body
        local flg = cfg.Flag or cfg.Name
        ...
        local svRow = new("Frame", {Size = UDim2.new(1, 0, 0, 100), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
        ...
        local hRow = new("Frame", {Size = UDim2.new(1, 0, 0, 20), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
        ...
        local hexRow = new("Frame", {Size = UDim2.new(1, 0, 0, 26), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
    ...
    function api:AddKeybind(cfg, target)
        target = target or body
        local flg = cfg.Flag or cfg.Name
    ...
    function api:AddLabel(cfg, target)
        target = target or body
        local lbl = tl({Size = UDim2.new(1, 0, 0, 20), BackgroundTransparency = 1, Text = cfg.Name or "", TextColor3 = T.MT, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, LayoutOrder = (target == body) and nl() or nil}, target)
        pad(0, 0, 12, 0, lbl)
    end

    function api:AddLabel2(cfg, target)
        target = target or body
        local lbl = tl({Size = UDim2.new(1, 0, 0, 20), BackgroundTransparency = 1, Text = cfg.Name or "", TextColor3 = T.A, TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left, LayoutOrder = (target == body) and nl() or nil}, target)
        pad(0, 0, 12, 0, lbl); table.insert(AL, function(c) lbl.TextColor3 = c end)
    end

    function api:AddDivider(target)
        target = target or body
        local wr = new("Frame", {Size = UDim2.new(1, 0, 0, 10), BackgroundTransparency = 1, LayoutOrder = (target == body) and nl() or nil}, target)
    ...
    function api:AddCanvas(cfg, target)
        target = target or body
        ...
        local wr = new("Frame", {
            ...
            LayoutOrder = (target == body) and nl() or nil,
        }, target)
        if cfg and cfg.Radius then cr(cfg.Radius, wr) end
        return wr
    end

    function api:AddScrollList(cfg, target)
        target = target or body
    ...
    function api:AddCodeBlock(cfg, target)
        target = target or body
    ...
    function api:AddSubTab(cfg, target)
        target = target or body
        local container = new("Frame", {
            Size = UDim2.new(1, 0, 0, (cfg and cfg.Height) or 120),
            BackgroundColor3 = T.B2,
            ClipsDescendants = true,
            LayoutOrder = (target == body) and nl() or nil,
        }, target)
        cr(4, container)
        local tabBtnsHolder = new("Frame", {
        ...
        new("UIListLayout", {FillDirection = Enum.FillDirection.Horizontal, Padding = UDim.new(0, 2), SortOrder = Enum.SortOrder.LayoutOrder}, tabBtnsHolder)
        pad(2, 0, 4, 4, tabBtnsHolder)
        local tabs = {}
        local activeSubTab = nil

        local function createSubSectionAPI(frame)
            return {
                AddButton = function(cfg) api:AddButton(cfg, frame) end,
                AddSubButton = function(cfg) api:AddSubButton(cfg, frame) end,
                AddToggle = function(cfg) return api:AddToggle(cfg, frame) end,
                AddSubToggle = function(cfg) return api:AddSubToggle(cfg, frame) end,
                AddLabel = function(cfg) api:AddLabel(cfg, frame) end,
                AddDivider = function() api:AddDivider(frame) end,
                AddCanvas = function(cfg) return api:AddCanvas(cfg, frame) end,
                AddScrollList = function(cfg) return api:AddScrollList(cfg, frame) end,
                AddCodeBlock = function(cfg) return api:AddCodeBlock(cfg, frame) end,
            }
        end

        local subTabGroup = {}
        function subTabGroup.AddTab(name)
            local idx = #tabs + 1
            local btn = new("TextButton", {
            ...
            new("UIListLayout", {SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 3)}, frame)
            local function switchTo()
                for i, f in ipairs(tabs) do
                    if i == idx then
                        f.frame.Visible = true
                        tw(f.btn, {TextColor3 = T.A}, 0.1)
                    else
                        f.frame.Visible = false
                        tw(f.btn, {TextColor3 = T.MT}, 0.1)
                    end
                end
                activeSubTab = idx
            end
            btn.MouseButton1Click:Connect(switchTo)
            table.insert(tabs, {btn = btn, frame = frame, switchTo = switchTo})
            if idx == 1 then switchTo() end
            return createSubSectionAPI(frame)
        end
        return subTabGroup
    end
...
local function showLoader(name, onDone)
    ...
    loaderGui.Name = "EtherLoader"; loaderGui.ResetOnSpawn = false; loaderGui.IgnoreGuiInset = true; loaderGui.ZIndexBehavior = Enum.ZIndexBehavior.Global; loaderGui.DisplayOrder = 999
...
local function Setup(cfg)
    ...
    local keys = cfg.Keys or {}; local keyURL = cfg.KeyURL or ""; local keyFile = cfg.KeyFile or "EtherKey.txt"; local persist = cfg.KeyPersist ~= false
