...
local T = {
    ...
    B1 = Color3.fromRGB(17, 17, 24),
    B2 = Color3.fromRGB(23, 23, 32),
    B3 = Color3.fromRGB(30, 30, 42),
    B4 = Color3.fromRGB(40, 40, 56),
    BD = Color3.fromRGB(52, 52, 72),
    TX = Color3.fromRGB(242, 242, 255),
    MT = Color3.fromRGB(110, 110, 145),
    SUB = Color3.fromRGB(65, 65, 90),
}
...
for _, v in ipairs({ game:GetService("CoreGui"), LP.PlayerGui }) do
    pcall(function()
        if v:FindFirstChild("EtherUI") then v.EtherUI:Destroy() end
    end)
end
...
local CFG_PREFIX = "EtherUI_cfg_"
local CFG_INDEX = "EtherUI_cfglist.json"
local CFG_AUTO = "EtherUI_autoload.txt"
local CFG_FOLDER = "EtherUI"
...
local function _encode()
    ...
    for k, v in pairs(Flags) do
        ...
        elseif t == "Color3" then
            out[k] = { r = v.R, g = v.G, b = v.B, __type = "Color3" }
        elseif t == "EnumItem" then
            out[k] = { name = v.Name, __type = "EnumItem", enumType = tostring(v.EnumType) }
        elseif t == "table" then
            out[k] = v
        end
    end
    ...
    for k, v in pairs(data) do
        if type(v) == "table" then
            ...
            elseif v.__type == "EnumItem" then
                pcall(function() Flags[k] = Enum[v.enumType][v.name] end)
            else
                Flags[k] = v
            end
        else
...
local function cfgSave(name)
    ...
    pcall(function()
        ...
        if not found then
            ...
            writefile(CFG_INDEX, HS:JSONEncode(list))
        end
        success = true
    end)
    return success
...
local function cfgSetAuto(name)
    pcall(function()
        if writefile then writefile(CFG_AUTO, name or "") end
    end)
end
...
local function cfgClearAuto()
    pcall(function()
        if isfile and isfile(CFG_AUTO) and delfile then delfile(CFG_AUTO) end
    end)
end

local _auto = cfgGetAuto()
if _auto then cfgLoad(_auto) end
...
local function new(cls, props, parent)
    ...
    if _TC[cls] then
        pcall(function() o.Font = FONT end)
    end
...
pcall(function() SG.Name = "EtherUI" end)
...
for i = 1, 28 do
    ...
    local f = new("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(255, 182, 217),
    ...
    flakes[i] = {
        ...
        y = math.random(-10, 110) / 100,
        spd = math.random(3, 9) / 100,
        dx = math.random(-12, 12) / 10000,
    }
...
local titleSep = new("Frame", {
    ...
    BackgroundColor3 = Color3.fromRGB(255, 205, 228),
...
local mbBar = new("Frame", {
    AnchorPoint = Vector2.new(0.5, 0.5),
    Position = UDim2.new(0.5, 0, 0.5, 0),
    Size = UDim2.new(0, 10, 0, 2),
    BackgroundColor3 = T.MT,
    ZIndex = 14,
}, mb)
cr(99, mbBar)

mb.MouseEnter:Connect(function()
    tw(mb, { BackgroundColor3 = T.B4 })
    tw(mbBar, { BackgroundColor3 = T.A })
end)
mb.MouseLeave:Connect(function()
    tw(mb, { BackgroundColor3 = T.B3 })
    tw(mbBar, { BackgroundColor3 = T.MT })
end)
...
UIS.InputChanged:Connect(function(i)
    if drag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
        ...
        Win.Position = UDim2.new(
            0, math.clamp(winStart.X.Offset + d.X, 0, math.max(0, vp.X - ws.X)),
            0, math.clamp(winStart.Y.Offset + d.Y, 0, math.max(0, vp.Y - ws.Y))
        )
    end
end)
...
local function setVisible(v)
    ...
    if v then
        ...
        if overlayEnabled then
            ...
            tw(bgOverlay, { BackgroundTransparency = 0.55 }, 0.2)
        end
        snowCont.Visible = true
    else
        Win.Visible = false
...
local function addTab(name)
    ...
    local function makeCol(xs, xo, wo)
        local col = new("ScrollingFrame", {
            Position = UDim2.new(xs, xo, 0, 6),
            Size = UDim2.new(0.5, wo, 1, -12),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
    ...
    local colL = makeCol(0, 8, -14)
    local colR = makeCol(0.5, 6, -14)
    table.insert(tabFrames, frame)
    table.insert(tabBtns, { b = btn, u = ul, idx = idx })
    ...
    local function switchToTab()
        ...
        for _, bd in ipairs(tabBtns) do
            if bd.idx == idx then
                tw(bd.b, { TextColor3 = T.TX }, 0.12)
                bd.u.Visible = true
                tw(bd.u, { BackgroundColor3 = T.A }, 0.12)
            else
                tw(bd.b, { TextColor3 = T.MT }, 0.12)
                bd.u.Visible = false
            end
        end
        activeTabIndex = idx
    ...
    btn.MouseEnter:Connect(function()
        if activeTabIndex ~= idx then tw(btn, { TextColor3 = T.TX }, 0.1) end
    end)
    btn.MouseLeave:Connect(function()
        if activeTabIndex ~= idx then tw(btn, { TextColor3 = T.MT }, 0.1) end
    end)
...
UIS.InputChanged:Connect(function(i)
    if slideActive and i.UserInputType == Enum.UserInputType.MouseMovement then
        local cb = slideCBs[slideActive]
        if cb then cb(i.Position.X, i.Position.Y) end
    end
end)
UIS.InputEnded:Connect(function(i)
...
local function regSlide(id, startFn, moveFn, obj)
    ...
    obj.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 then
            slideActive = id
            startFn(i.Position.X, i.Position.Y)
        end
    end)
end

local function buildElements(api, body, nl, tabName, tabSwitch, indent)
    indent = indent or 0
    local leftPad = 12 + indent * 14

    function api:AddSubTitle(cfg)
        local wr = new("Frame", {
        ...
        local pip = new("Frame", {
            AnchorPoint = Vector2.new(0, 0.5),
            Position = UDim2.new(0, leftPad - 6, 0.5, 0),
        ...
        tl({
            Position = UDim2.new(0, leftPad + 2, 0, 0),
    ...
    function api:AddCheckbox(cfg)
        ...
        table.insert(searchItems, {
            label = cfg.Name, keywords = cfg.Name .. (cfg.Flag or ""),
            tab = tabName, switch = tabSwitch,
        })
        local row = new("Frame", {
        ...
        tl({
            ...
            Size = UDim2.new(1, -leftPad - 34, 1, 0),
            BackgroundTransparency = 1,
            Text = cfg.Name,
            TextColor3 = T.TX,
            TextSize = 12,
        ...
        local function upd()
            if on then
                cbFill.Visible = true
                tw(cbSt, { Color = T.A, Thickness = 1.5 })
                tw(cbFill, { BackgroundColor3 = T.A })
            else
                cbFill.Visible = false
                tw(cbSt, { Color = T.BD, Thickness = 1.5 })
            end
        end
        table.insert(AL, function(c)
            if on then
                cbFill.BackgroundColor3 = c
                cbSt.Color = c
            end
        end)
        local btn = new("TextButton", {
    ...
    function api:AddToggle(cfg)
        ...
        tl({
            Position = UDim2.new(0, leftPad, 0, 0),
            Size = UDim2.new(1, -leftPad - 54, 1, 0),
            BackgroundTransparency = 1,
            Text = cfg.Name,
            TextColor3 = T.TX,
            TextSize = 12,
        ...
        local function upd(anim)
            ...
            if on then
                tw(track, { BackgroundColor3 = T.A }, t)
                tw(trkSt, { Color = T.A }, t)
                tw(knob, { Position = UDim2.new(1, -15, 0.5, 0), BackgroundColor3 = Color3.new(1, 1, 1) }, t, es)
            else
                tw(track, { BackgroundColor3 = T.B1 }, t)
        ...
        local function _syncSiblings(v)
            for _, fn in ipairs(togByFlag[flg]) do pcall(fn, v, true) end
        end
        if cfg.Keybind then
    ...
    function api:AddSubToggle(cfg)
        cfg = cfg or {}
        local oldIndent = indent
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local t = self:AddToggle(cfg)
        leftPad = oldLeftPad
        indent = oldIndent
        return t
    end
    ...
    function api:AddSlider(cfg)
        ...
        local function fmt(v)
            return dec > 0 and string.format("%." .. dec .. "f", v) or tostring(math.round(v))
        end
        local wr = new("Frame", {
        ...
        cr(4, hbg)
        tl({
            Position = UDim2.new(0, leftPad, 0, 5),
            Size = UDim2.new(0.6, -leftPad - 4, 0, 16),
            BackgroundTransparency = 1,
            Text = cfg.Name,
            TextColor3 = T.TX,
            TextSize = 12,
        ...
        local trackHit = new("Frame", {
            Position = UDim2.new(0, leftPad, 0, 28),
            Size = UDim2.new(1, -leftPad - 12, 0, 12),
            BackgroundTransparency = 1,
        }, wr)
        local track = new("Frame", {
            AnchorPoint = Vector2.new(0, 0.5),
        ...
        local function setVal(v)
            ...
            tw(fil, { Size = UDim2.new(p, 0, 1, 0) }, 0.05)
        ...
        eb.FocusLost:Connect(function()
            eb.Visible = false
            local n = tonumber(eb.Text)
            if n then setVal(n) end
            vl.Text = fmt(val)
        end)
        local function fromX(x)
            setVal(cfg.Min + math.clamp((x - track.AbsolutePosition.X) / track.AbsoluteSize.X, 0, 1) * (cfg.Max - cfg.Min))
        end
    ...
    function api:AddDropdown(cfg)
        local flg = cfg.Flag or cfg.Name
        local dv = Flags[flg] or cfg.Default or (cfg.Items and cfg.Items[1]) or ""
        Flags[flg] = dv
        table.insert(searchItems, {
            label = cfg.Name, keywords = cfg.Name .. (cfg.Flag or ""),
            tab = tabName, switch = tabSwitch,
        ...
        tl({
            Position = UDim2.new(0, leftPad, 0, 4),
            Size = UDim2.new(1, -leftPad - 12, 0, 14),
            BackgroundTransparency = 1,
            Text = cfg.Name,
            TextColor3 = T.TX,
            TextSize = 12,
        ...
        local sb = new("TextButton", {
            Position = UDim2.new(0, leftPad - 2, 0, 20),
            Size = UDim2.new(1, -leftPad - 10, 0, 22),
            BackgroundColor3 = T.B1,
            Text = "",
            ZIndex = 5,
        }, wr)
        ...
        local function closeDd()
            ...
            if lf then
                tw(lf, { Size = UDim2.new(1, -leftPad - 10, 0, 0) }, 0.12)
                tw(wr, { Size = UDim2.new(1, 0, 0, 46) }, 0.12)
                task.delay(0.13, function() if lf then lf:Destroy() lf = nil end end)
            end
        end
        sb.MouseButton1Click:Connect(function()
            ...
            if open then
                ...
                lf = new("Frame", {
                    Position = UDim2.new(0, leftPad - 2, 0, 44),
                    Size = UDim2.new(1, -leftPad - 10, 0, 0),
                    BackgroundColor3 = T.B1,
                    ClipsDescendants = true,
                    ZIndex = 20,
                }, wr)
                ...
                for _, item in ipairs(cfg.Items or {}) do
                    local op = new("TextButton", {
                        ...
                        BackgroundColor3 = T.B1,
                        Text = "  " .. tostring(item),
                        TextColor3 = item == Flags[flg] and T.A or T.TX,
                        TextSize = 12,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        ZIndex = 21,
                    }, lf)
                ...
                tw(lf, { Size = UDim2.new(1, -leftPad - 10, 0, h) }, 0.14)
                tw(wr, { Size = UDim2.new(1, 0, 0, 46 + h + 2) }, 0.14)
        ...
        local obj = {
            Get = function() return Flags[flg] end,
            Set = function(v)
                if v then
                    Flags[flg] = v
        ...
        function obj:Rebuild(items)
            cfg.Items = items
            Flags[flg] = items[1] or "(none)"
            selLbl.Text = tostring(Flags[flg])
        end
        return obj
    end

    function api:AddSubDropdown(cfg)
        cfg = cfg or {}
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local d = self:AddDropdown(cfg)
        leftPad = oldLeftPad
        return d
    end
    ...
    function api:AddButton(cfg)
        table.insert(searchItems, {
            label = cfg.Name, keywords = cfg.Name,
            tab = tabName, switch = tabSwitch,
        })
        local wr = new("Frame", {
            Size = UDim2.new(1, 0, 0, 32),
        ...
        local btn = new("TextButton", {
            Position = UDim2.new(0, leftPad - 2, 0.5, -11),
            Size = UDim2.new(1, -leftPad - 10, 0, 22),
            BackgroundColor3 = T.B1,
            Text = cfg.Name,
            TextColor3 = T.MT,
        ...
        btn.MouseEnter:Connect(function()
            tw(btn, { BackgroundColor3 = T.B3, TextColor3 = T.TX })
            tw(bSt, { Color = T.A }, 0.1)
        end)
        btn.MouseLeave:Connect(function()
            tw(btn, { BackgroundColor3 = T.B1, TextColor3 = T.MT })
            tw(bSt, { Color = T.BD }, 0.1)
        end)
    ...
    function api:AddSubButton(cfg)
        cfg = cfg or {}
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local b = self:AddButton(cfg)
        leftPad = oldLeftPad
        return b
    end
    ...
    function api:AddTextBox(cfg)
        ...
        tb.FocusLost:Connect(function(e)
            tw(tSt, { Color = T.BD, Thickness = 1 })
            Flags[flg] = tb.Text
            if e and cfg.Callback then cfg.Callback(tb.Text) end
        end)
        return {
            Get = function() return tb.Text end,
            Set = function(v) tb.Text = v Flags[flg] = v end,
        }
    end
    ...
    function api:AddColorPicker(cfg)
        local flg = cfg.Flag or cfg.Name
        local sv = Flags[flg]
        local cur = (typeof(sv) == "Color3" and sv) or cfg.Default or T.A
        Flags[flg] = cur
        table.insert(searchItems, {
            label = cfg.Name, keywords = cfg.Name .. (cfg.Flag or ""),
            tab = tabName, switch = tabSwitch,
        ...
        tl({
            Position = UDim2.new(0, leftPad, 0, 0),
            Size = UDim2.new(1, -leftPad - 46, 1, 0),
            BackgroundTransparency = 1,
            Text = cfg.Name,
            TextColor3 = T.TX,
            TextSize = 12,
        ...
        local svBox = new("Frame", {
            Position = UDim2.new(0, leftPad, 0, 4),
            Size = UDim2.new(1, -leftPad - 12, 0, 84),
        ...
        local function applySV(x, y)
            ...
            svT.Position = UDim2.new(cS, 0, 1 - cV, 0)
            rebuild()
            sw.BackgroundColor3 = cur
        end
        regSlide("sv_" .. flg, applySV, applySV, svBox)
        local hRow = new("Frame", {
            Size = UDim2.new(1, 0, 0, 18),
            BackgroundTransparency = 1,
            LayoutOrder = nl(),
        }, body)
        local hTrk = new("Frame", {
            Position = UDim2.new(0, leftPad, 0.5, -3),
            Size = UDim2.new(1, -leftPad - 12, 0, 6),
            BackgroundColor3 = Color3.new(1, 1, 1),
        }, hRow)
        cr(99, hTrk)
        new("UIGradient", {
        ...
        local function applyH(x)
            ...
            hT.Position = UDim2.new(cH, 0, 0.5, 0)
    ...
    function api:AddCodeBlock(cfg)
        ...
        local codeLbl = tl({
            ...
            BackgroundTransparency = 1,
            Text = '<font color="rgb(75,80,105)">-- empty</font>',
            TextColor3 = T.TX,
            Font = Enum.Font.Code,
            TextSize = 10,
            TextXAlignment = Enum.TextXAlignment.Left,
    ...
    function api:AddKeybind(cfg)
        ...
        tl({
            Position = UDim2.new(0, leftPad, 0, 0),
            Size = UDim2.new(1, -leftPad - 92, 1, 0),
            BackgroundTransparency = 1,
            Text = cfg.Name,
            TextColor3 = T.TX,
            TextSize = 12,
        ...
        badge.MouseButton1Click:Connect(function()
            ...
            badge.Text = "..."
            badge.TextColor3 = T.MT
            tw(badge, { BackgroundColor3 = T.B3 })
            tw(bSt, { Color = T.A })
            local conn
            conn = UIS.InputBegan:Connect(function(i)
                if i.UserInputType == Enum.UserInputType.Keyboard then
                    ...
                    badge.Text = cur.Name
                    badge.TextColor3 = T.A
                    tw(badge, { BackgroundColor3 = T.B1 })
                    tw(bSt, { Color = T.BD })
                    if cfg.Callback then cfg.Callback(cur) end
                    conn:Disconnect()
                end
    ...
    function api:AddSubKeybind(cfg)
        cfg = cfg or {}
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local k = self:AddKeybind(cfg)
        leftPad = oldLeftPad
        return k
    end

    function api:AddSubCheckbox(cfg)
        cfg = cfg or {}
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local c = self:AddCheckbox(cfg)
        leftPad = oldLeftPad
        return c
    end

    function api:AddSubSlider(cfg)
        cfg = cfg or {}
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local s = self:AddSlider(cfg)
        leftPad = oldLeftPad
        return s
    end

    function api:AddSubTextBox(cfg)
        cfg = cfg or {}
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local t = self:AddTextBox(cfg)
        leftPad = oldLeftPad
        return t
    end

    function api:AddSubColorPicker(cfg)
        cfg = cfg or {}
        local oldLeftPad = leftPad
        leftPad = leftPad + 14
        local c = self:AddColorPicker(cfg)
        leftPad = oldLeftPad
        return c
    end

    function api:AddSubDivider()
        local wr = new("Frame", {
        ...
        new("Frame", {
            ...
            Size = UDim2.new(1, -leftPad - 12, 0, 1),
            BackgroundColor3 = T.BD,
            BackgroundTransparency = 0.6,
        }, wr)
    end

    function api:AddSubTab(title)
        ...
        local activeSub = nil
        local subObj = {}
        function subObj:AddSection(subName)
            local idx = #subTabs + 1
            ...
            local subApi = {}
            buildElements(subApi, panel, function() local o = 0 return function() o += 1 return o end end end, tabName, tabSwitch, indent + 1)
            local entry = { btn = sBtn, ul = ul, panel = panel, api = subApi }
            table.insert(subTabs, entry)
            sBtn.MouseButton1Click:Connect(function()
                for _, e in ipairs(subTabs) do
                    e.panel.Visible = false
                    e.ul.Visible = false
                    tw(e.btn, { TextColor3 = T.MT }, 0.12)
                end
                panel.Visible = true
                ul.Visible = true
                tw(sBtn, { TextColor3 = T.TX }, 0.12)
                tw(ul, { BackgroundColor3 = T.A }, 0.12)
            end)
            if #subTabs == 1 then
                panel.Visible = true
                ul.Visible = true
...
local function makeSection(parent, title)
    ...
    buildElements(api, body, function() local o = 0 return function() o += 1 return o end end end, tabName, tabSwitch, 0)
    return api
end
...
local wmIconImg = new("ImageLabel", {
...
local function Setup(cfg)
    ...
    if cfg.Snow == true then
        startSnow()
    elseif cfg.Snow == false then
        stopSnow()
    end
    local keys = cfg.Keys or {}
    local keyURL = cfg.KeyURL or ""
    local keyFile = cfg.KeyFile or "EtherKey.txt"
    ...
    local function revealUI()
        ...
        Win.Visible = true
        Win.BackgroundTransparency = 1
        tw(Win, { BackgroundTransparency = 0 }, 0.3, Enum.EasingStyle.Quad)
        if overlayEnabled then
            bgOverlay.Visible = true
            bgOverlay.BackgroundTransparency = 0.9
            tw(bgOverlay, { BackgroundTransparency = 0.55 }, 0.3)
        end
        if WM_SHOW then
            wmFrame.Visible = true
            wmFrame.BackgroundTransparency = 1
            tw(wmFrame, { BackgroundTransparency = 0.08 }, 0.3)
        end
        snowCont.Visible = true
    end
    ...
    if ICON_ID ~= "" then
        ...
        cr(99, ad:FindFirstChildOfClass("Frame"))
    end
    tl({
        AnchorPoint = Vector2.new(0.5, 0),
    ...
    if persist then
        tl({
            ...
            Size = UDim2.new(1, -40, 0, 11),
            BackgroundTransparency = 1,
            Text = "key saved for next session",
            TextColor3 = T.SUB,
            TextSize = 10,
            TextXAlignment = Enum.TextXAlignment.Left,
            ZIndex = 202,
    ...
    local getBtn = new("TextButton", {
        Position = UDim2.new(0.5, 4, 0, btnY),
        Size = UDim2.new(0.5, -20, 0, 34),
        BackgroundColor3 = T.B3,
        Text = "Get Key",
        TextColor3 = T.TX,
    ...
    local function tryKey()
        ...
        if valid then
            ...
            tw(ov, { BackgroundTransparency = 1 }, 0.3)
            tw(md, { BackgroundTransparency = 1 }, 0.3)
            ...
            el.Text = "incorrect key, try again"
            tw(iSt, { Color = Color3.fromRGB(200, 60, 60), Thickness = 1.5 }, 0.1)
            task.delay(2, function()
                el.Text = ""
                tw(iSt, { Color = T.BD, Thickness = 1 }, 0.2)
            end)
        end
    end
...
return {
    ...
    avImg = avImg,
    wmFrame = wmFrame,
    wmNameLabel = wmNameLabel, -- not used but kept
    SaveConfig = cfgSave,
    LoadConfig = cfgLoad,
